...
const Education = () => {
   ...
   const datas = allData?.[path]?.education
    const width = useWidth()
    return (
        <>
        {
            datas && datas.length>0?  <div className={`${width>1280?'mt-[2px]':"mt-4"}`}>
                        ...
                        datas.map((item)=>{ 
                            return(
                                <div key={item.id} className={`${width>1280?'':"mt-1"}`}>
                                    <h2 className={`font-semibold edu_tittle ${width>1280?'text-[8px]':'text-xl'}`}>{item?.school}</h2>
                                    <h2 className={`font-semibold text-gray-400 edu_year ${width>1280?'text-[8px]':''}`}>{`${
                                        item?.startDate 
                                    } - ${item?.endDate}`}</h2>
                                    {
                                    ...
                                    <h2 className={`text-gray-400 edu_skill ${width>1280?'ml-2 text-[7px]':'ml-4'}`}>{item?.summary}</h2>
