        ...
        <div className=' col-span-5'>
           {
                ...
                <h2 className={`${width>=1280 || width<=480 || width<=768?"text-[16px]":"text-3xl  mb-3"} font-bold text-[#1B3055]`}>
                           ...
                           {
                               skills.map((skill,index)=> <div className={`flex justify-between items-center  ${width>=1280 || width<=480 || width<=768?"gap-x-[2px]":"gap-3"}  text-gray-700`} key={skill?.id}>
                               <h2 className={`font-bold skill_tittle ${width>=1280 || width<=480 || width<=768?"text-[8px]":""}`}  key={index}>{skill?.tittle}</h2>
                                <div className={` w-auto border-gray-700 h-0 flex-grow ${width>=1280 || width<=480 || width<=768?' border-[1px]':" border"}`}></div>
                                <h2 className={`font-medium skill_rating ${width>=1280 || width<=480 || width<=768?"text-[8px]":""}`}>{skill?.rating ==='expert'?5 : skill?.rating==='intermediate' ? 4 : skill?.rating==='beginner'?3:5}/5</h2>
                               </div>)   
                           }
                </div>
                :null
