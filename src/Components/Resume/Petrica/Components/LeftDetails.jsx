        ...
        <div className=' col-span-4'>
            {
                ...
                <h2 className={`font-bold text-[#1b3055] ${width>1280?"text-[10px]":"text-xl"}`}>Skills</h2>
                 {
                        skills.map((skill,i) => (
                            <div key={i} className={`skills ${width>1280?"py-1":"py-2"}`}>
                                <h2 className={`font-semibold text-[#1b3055] skill_tittle ${width>1280?"text-[8px] mb-1":""}`}>{skill}</h2>
                                  <div className={` flex ${width>1280?"gap-1":'gap-2'}`}>
                                  {
                                        [...Array(5)].map((_,i) => <div  className={`${width>1280?"h-[2px]":"h-1 mt-1"} bg-[#181b1a] w-full  skill_rating`}></div>) 
                                  }
                                  </div>
                            </div>
                        ))
            ...
            <div className={`${width>1280?' mt-2':' mt-8'}`}>
               <>
               {
                language && language.length>0 ? <>  <h2 className={`font-bold text-[#1b3055] ${width>1280?"text-[10px]":"text-xl"}`}>Language</h2>
                {
                       language.map(lan => (
