                        ...
                        skills.map((skill,i) => (
                            <div key={i} className={`skills ${width>1280?"py-1":"py-2"}`}>
                                <h2 className={`font-semibold text-[#1b3055] skill_tittle ${width>1280?"text-[8px] mb-1":""}`}>{skill?.tittle}</h2>
                                  ...
                                  {
                                        [...Array(skill?.rating ==='expert'?5 : skill?.rating==='intermediate' ? 4 : skill?.rating==='beginner'?3:5)].map((_,i) => <div  className={`${width>1280?"h-[2px]":"h-1 mt-1"} bg-[#181b1a] w-full  skill_rating`}></div>) 
                                  }
                                  </div>
                            </div>
                        ))
